...
while true
  ...
  # Go left
  if node[:moves].last != 'r' # last move cannot be left!
    i = node[:i]
    j = node[:j] - 1
    if j >= 0
      next_nodes << {
  ...
  # Go right
  if node[:moves].last != 'l' # last move cannot be right!
    i = node[:i]
    j = node[:j] + 1
  ...
  # Go up
  if node[:moves].last != 'd' # last move cannot be down!
    i = node[:i] - 1
    j = node[:j]
    if i >= 0
      next_nodes << {
  ...
  # Go down
  if node[:moves].last != 'u' # last move cannot be up!
    i = node[:i] + 1
    j = node[:j]
    if i < block.size
      next_nodes << {
  ...
  # Reject potential nodes with 4 moves in same direction!
  next_nodes = next_nodes.reject do |next_node|
    next_node[:moves].size >= 4 && next_node[:moves].last(4).uniq.size == 1
  end
